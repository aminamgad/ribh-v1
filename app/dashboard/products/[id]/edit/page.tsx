...
import { z } from 'zod';
import { Save, Package, ImageIcon, DollarSign, Warehouse, Layers, Eye, ChevronLeft, ChevronRight, X, AlertCircle, CheckCircle2, Info, Search, Plus, ArrowLeft, Tag } from 'lucide-react';
...
// Steps configuration - 5 steps with Variants as separate step (matching new product page)
const STEPS = [
...
export default function EditProductPage() {
  ...
  const fetchProduct = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        setValue('supplierPrice', data.product.supplierPrice || 0);
        ...
        // Load tags and specifications
        setTags(data.product.tags || []);
        if (data.product.specifications) {
          const specs = Object.entries(data.product.specifications).map(([key, value]) => ({
            key,
            value: String(value)
          }));
        ...
        // Load supplier if admin
        if (user?.role === 'admin' && data.product.supplierId) {
          setSelectedSupplierId(data.product.supplierId);
        }
        
        // Mark initial steps as completed
        const initialCompleted = new Set<number>();
        if (data.product.name && data.product.name.length >= 3) initialCompleted.add(1);
        if (productImages.length > 0) initialCompleted.add(2);
        if (data.product.marketerPrice > 0) initialCompleted.add(3);
        if (data.product.stockQuantity >= 0) initialCompleted.add(4);
        if (data.product.hasVariants !== undefined) initialCompleted.add(5);
        setCompletedSteps(initialCompleted);
      } else if (response.status === 403) {
        // Access denied - supplier trying to access another supplier's product
        const errorData = await response.json().catch(() => ({}));
        toast.error(errorData.message || 'غير مصرح لك بالوصول لهذا المنتج');
  ...
  // Convert Arabic text to English for SKU
  ...
  // Check SKU availability (exclude current product)
  const checkSKU = async (sku: string) => {
    ...
    try {
      const response = await fetch(`/api/products/check-sku?sku=${encodeURIComponent(sku)}&excludeId=${params.id}`);
      if (response.ok) {
        const data = await response.json();
        if (data.exists) {
    ...
    const subscription = watch((value, { name, type }) => {
      ...
      if (productName && productName.length >= 3) {
        const timeoutId = setTimeout(async () => {
          try {
            const response = await fetch(`/api/products/check-name?name=${encodeURIComponent(productName)}&excludeId=${params.id}`);
            if (response.ok) {
              const data = await response.json();
              if (data.exists) {
  ...
  // Check for draft after product is loaded
  useEffect(() => {
    if (!product || !params.id) return;
    
    const savedDraft = localStorage.getItem(`product-edit-draft-${params.id}`);
    if (savedDraft) {
      try {
        ...
        if (dontAsk === 'true') {
          ...
          localStorage.removeItem(`product-edit-draft-${params.id}`);
          return;
        }
  ...
  const handleRestoreDraft = useCallback(() => {
    ...
    try {
      ...
      setHasVariants(draftData.hasVariants !== undefined ? draftData.hasVariants : null);
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem(`product-edit-draft-${params.id}`, JSON.stringify(draftData));
  ...
  }, [hasUnsavedChanges, getValues, images, hasVariants, variants, variantOptions, selectedSupplierId, primaryImageIndex, currentStep, tags, specifications, params.id]);
  ...
  const calculateStatistics = useCallback(() => {
    ...
    const estimatedMinutes = Math.ceil(remainingFields * 0.5);
  ...
  const onSubmit = useCallback(async (data: ProductFormData) => {
    ...
    // Validate pricing
    if (data.wholesalerPrice && data.marketerPrice && data.wholesalerPrice >= data.marketerPrice) {
    ...
    if (data.minimumSellingPrice && data.marketerPrice && data.marketerPrice >= data.minimumSellingPrice) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`❌ ${result.message || 'غير مصرح لك بتعديل هذا المنتج'}`, {
          duration: 6000,
          style: {
            background: '#ef4444',
            color: '#fff',
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-[1920px] mx-auto px-2 sm:px-4 md:px-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">تعديل المنتج</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">عدّل منتجك في المنصة</p>
        </div>
        {isSaving && (
                  ...
                  {errors.minimumSellingPrice && (
                      ...
                      {minimumSellingPrice > 0 && marketerPrice < minimumSellingPrice && !errors.minimumSellingPrice && (
                        <p className="text-green-600 dark:text-green-400 text-xs sm:text-sm flex items-center gap-1.5">
                          ...
                          السعر الأدنى للبيع (السعر المقترح للمسوق) صحيح
                    </p>
                  )}
                </div>
            ...
            <div className="flex gap-2 sm:gap-3 flex-1 sm:flex-initial">
              {currentStep < STEPS.length ? (
                ...
                <button
                  ...
                  disabled={saving || uploading || !validateStep(5)}
                  className="btn-primary min-h-[48px] sm:min-h-[44px] text-base sm:text-sm flex items-center justify-center gap-2 flex-1 sm:flex-initial px-4 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {saving ? (
                    <>
