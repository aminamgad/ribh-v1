...
### 4. المنتجات المقترحة بعد إتمام الطلب (ربح ← Easy Orders)

| # | المهمة | الحالة | التفاصيل |
|---|--------|--------|----------|
| 4.1 | API المنتجات المقترحة | ✅ | `GET /api/orders/[id]/recommended-products` — منتجات من نفس المورد |
| 4.2 | عرض المنتجات بعد الطلب | ✅ | قسم "منتجات مقترحة - أكمل طلبك" في صفحة تفاصيل الطلب عند `?showRecommended=1` |
| 4.3 | إنشاء طلب جديد | ✅ | أضف للسلة ← إتمام الطلب — الطلب يصل إلى ربح بنجاح |
| 4.4 | تفضيل المصدرة لـ EO | ✅ | المنتجات المُصدَّرة إلى Easy Orders تظهر أولاً |

**المسار:** سلة → إتمام الطلب → صفحة الطلب + منتجات مقترحة → أضف للسلة → طلب جديد → ربح.
